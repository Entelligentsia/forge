...
function isNonInteractiveLocal() {
 ...
 */
export async function runPhase4(ctx4) {
    const { cwd, bundleRoot, toolsRoot, projectName, configCache, ctx, isPiRuntime, getBundledToolsRoot: getToolsRoot, } = ctx4;
    ...
    if (fs.existsSync(generationManifestTool)) {
        ...
        for (const mdFile of collectMdFiles(kbDir)) {
            await runToolAdvisory(generationManifestTool, ["record", mdFile], cwd, ctx, `step 4-3 generation-manifest seed: ${path.relative(cwd, mdFile)}`, 10000);
        }
        await runToolAdvisory(generationManifestTool, ["record-all"], cwd, ctx, "step 4-3 generation-manifest record-all", 30000);
    }
    ...
    try {
        ...
        if (refreshResult.filesUpdated === 0) {
            ctx.ui.notify("△ Run /forge:refresh-kb-links manually after init completes " + "(no agent instruction files found).", "info");
        }
    }
    catch (err) {
        const e = err;
    ...
    if (isPiRuntime()) {
        ...
        {
            const proj = configCache.project;
...
//# sourceMappingURL=phase4-register.js.map