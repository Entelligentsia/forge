...
function readPhasePrompt(bundleRoot, phaseNum) {
    ...
    if (!filename) {
        throw new Error(`Phase ${phaseNum} prompt file not found in ${phasesDir} (expected: ${pattern}*.md). ` +
...
async function renderPhaseBanner(toolsRoot, cwd, phaseNum, phaseName, bannerKey) {
    const bannersTool = path.join(toolsRoot, "banners.cjs");
    if (!fs.existsSync(bannersTool))
        return;
    await execFileAsync("node", [bannersTool, "--phase", String(phaseNum), "4", phaseName, bannerKey], { cwd, timeout: 5000 }).catch(() => {
        /* non-fatal */
    });
}
// ── Phase 1 — Collect ─────────────────────────────────────────────────────────
export async function runPhase1(cwd, bundleRoot, toolsRoot, configCache, ctx, sendToAgent, waitForIdle, isNonInteractive) {
    ...
    if (!result.ok) {
        const retrySteer = `Phase 1 verification failed. Missing: ${result.missing.join(", ")}.\n\n` +
    ...
    if (!result.ok) {
        ...
        const proceed = await ctx.ui.confirm("Phase 1 — continue?", `Phase 1 verify failed: ${result.missing.join(", ")}. Continue to next phase?`);
        if (!proceed) {
        ...
        if (!useDefault) {
            ...
            if (customName && customName.trim()) {
                ...
                if (fs.existsSync(manageConfigTool)) {
                    await runToolAdvisory(manageConfigTool, ["set", "paths.engineering", customName.trim()], cwd, ctx, "manage-config paths.engineering");
                }
            }
        }
    }
...
// ── Phase 2 — Discover ────────────────────────────────────────────────────────
export async function runPhase2(cwd, bundleRoot, toolsRoot, projectName, configCache, ctx, sendToAgent, waitForIdle, isNonInteractive) {
    ...
    // resolves before the agent finishes; no warning to avoid confusing the user)
    if (!result.ok) {
        const retrySteer = `Phase 2 verification failed. Missing KB docs: ${result.missing.join(", ")}.\n\n` +
    ...
    if (!result.ok) {
        ...
        const proceed = await ctx.ui.confirm("Phase 2 — continue?", `Phase 2 verify failed: ${result.missing.join(", ")}. Continue to next phase?`);
        if (!proceed) {
...
export async function runPhase3(cwd, bundleRoot, toolsRoot, ctx) {
    ...
    try {
        ...
        if (cfg?.paths?.engineering)
            kbPath = cfg.paths.engineering;
    }
    catch {
    ...
    if (!result.ok) {
        ctx.ui.notify(`× Phase 3 failed: ${result.missing.join(", ")}. ` +
            `This usually means substitute-placeholders.cjs ran against an incomplete config. ` +
            `Fix .forge/config.json and run /forge:rebuild, or restart /forge:init from scratch (delete .forge/init-progress.json).`, "error");
        return "abort";
    }
...
//# sourceMappingURL=run-phases.js.map