...
export async function runWorkflow(opts) {
    ...
    fs.writeFileSync(path.join(workingDir, "manifest.json"), JSON.stringify({
        workflowId: wf.id,
        version: wf.version,
        instanceId,
    ...
    for (const n of wf.nodes) {
        if (n.loop?.group && n.loop?.head)
            groupHeads.set(n.loop.group, n.id);
    }
    // Main loop
    while (true) {
        ...
        if (node.loop) {
            ...
            const collection = resolvePath(state, node.loop.over);
            ...
            const itemId = item.id ?? String(iter);
        ...
        const onEvent = (event) => {
            ...
            switch (e.type) {
                ...
                case "tool_execution_end": {
                    ...
                    registry?.appendTail(instanceId, role, e.isError ? `⚠ ${toolName} failed` : `← ${toolName} ok`, e.isError ? { warning: true } : undefined);
                    break;
                }
            }
        };
        ...
        const stamped = parsed.events.map((e, i) => ({
            ...e,
        ...
        if (edge.advance === "loop-or-next") {
            ...
            if (loopCtx?.group) {
                ...
                if (!head)
                    throw new Error(`group ${loopCtx.group} has no head`);
...
function setPath(obj, dotted, value) {
    ...
    for (let i = 0; i < parts.length - 1; i++) {
        ...
        if (Array.isArray(cur)) {
            ...
            const rec = cur;
            if (typeof rec[p] !== "object" || rec[p] === null)
                rec[p] = {};
            cur = rec[p];
        }
        else {
