...
const REQUIRED_PATHS = ["engineering", "store", "workflows", "commands", "templates"];
// ── Main function ──────────────────────────────────────────────────────────
/**
...
export async function runHealthCheck(cwd, bundleRoot, forgeRoot) {
    ...
    if ("project" in configRecord && configRecord.project && typeof configRecord.project === "object") {
        const project = configRecord.project;
    ...
    if ("paths" in configRecord && configRecord.paths && typeof configRecord.paths === "object") {
        const paths_ = configRecord.paths;
    ...
    const calibrationBaseline = configRecord.calibrationBaseline;
    if (!calibrationBaseline) {
        gaps.push({
            check: "kb-freshness",
            severity: "warning",
            message: "No calibration baseline found — run /forge:health --fix to establish one." +
        ...
        const engPath = configRecord.paths?.engineering ?? "engineering";
        ...
        try {
            ...
            if (currentHash !== calibrationBaseline.masterIndexHash) {
                ...
                const message = `KB freshness: MASTER_INDEX.md hash has changed since last calibration — run /forge:health --fix to re-align.${driftDetail}` +
