...
const _syntheticHandlers = new Map();
/**
 ...
 */
export function onSyntheticEvent(eventType, handler) {
 ...
 */
export async function emitSyntheticEvent(event, ctx) {
 ...
 */
export function _resetSyntheticHandlers() {
...
function appendAudit(logsDir, line) {
    if (!auditEnabled())
        return;
    try {
 ...
 */
export function parseStoreCLIInvocation(command, _forgeRoot) {
    // Quick pre-filter before any parsing overhead.
    if (!command.includes("store-cli.cjs"))
        return null;
    ...
    if (storeCliIdx === -1 || storeCliIdx + 1 >= tokens.length)
        return null;
 ...
 */
function tokeniseShellCommand(command) {
    const tokens = [];
    let i = 0;
 ...
 */
export function registerHookDispatcher(pi, forgeRoot) {
    ...
    pi.on("tool_call", (event) => {
        ...
        if (isToolCallEventType("bash", event)) {
            const bashEvent = event;
            const intercept = parseStoreCLIInvocation(bashEvent.input.command, forgeRoot);
            if (intercept) {
                appendAudit(logsDir, `[store-cli-intercept] subcmd=${intercept.subcmd} entity=${intercept.entity} payload=${JSON.stringify(intercept.payload)}`);
                ...
                if (intercept.subcmd === "write") {
                    ...
                    if (!hasForce) {
                        const payloadRecord = intercept.payload;
                        ...
                        if (guard.reason === "lookup-failed") {
                            ...
                            appendAudit(logsDir, `[store-cli-intercept] decision=lookup-failed entity=${intercept.entity} entityId=${payloadRecord.entityId}`);
                            return undefined;
                        }
                        if (!guard.allowed) {
                        ...
                        appendAudit(logsDir, `[store-cli-intercept] decision=would-allow (--force bypasses transition-guard)`);
                    }
                }
            }
        }
