 ...
 */
export function loadForgePersona(name, cwd) {
 ...
 */
export async function runForgeSubagent(opts) {
    ...
    if (opts.requestedModel) {
        ...
        if (m) {
            try {
                ...
                const e = err;
                process.stderr.write(`[forge-subagent] setModel failed for ${opts.requestedModel.provider}:${opts.requestedModel.model} (non-fatal, inheriting pi current model): ${e.message ?? "unknown"}\n`);
            }
        }
        else {
    ...
    const unsubscribe = session.subscribe((event) => {
        ...
        if (event.type === "turn_end" && event.message) {
            ...
            if (msg.role === "assistant") {
                ...
                if (!result.provider && msg.provider)
 ...
 */
export function formatTranscriptTimestamp(d) {
 ...
 */
export function computeTranscriptPath(cwd, persona, tag, startedAt) {
...
export function writeSubagentTranscript(opts) {
...
//# sourceMappingURL=forge-subagent.js.map