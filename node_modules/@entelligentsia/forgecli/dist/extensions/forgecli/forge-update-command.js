...
function changelogTagUrl(version) {
...
export function detectInstallMethod(opts) {
    ...
    if (/[/\\]_npx[/\\]/.test(norm))
        return "npx";
    if (opts.globalRoot && norm.startsWith(path.resolve(opts.globalRoot)))
        return "global";
    return "local-dev";
}
export async function getNpmGlobalRoot(runner) {
    const run = runner ?? execFileAsync;
    try {
...
export async function fetchChangelog(fetchImpl) {
...
export function isUpgrade(current, latest) {
...
export function composeChangelogSummary(current, latest, body) {
...
export async function runUpgrade(spec, runner) {
    const run = runner ?? execFileAsync;
    try {
...
function driftCachePath(dir) {
    return path.join(dir, DRIFT_CACHE_FILE);
}
async function readDriftCache(dir) {
    try {
        const raw = await fs.readFile(driftCachePath(dir), "utf8");
        const parsed = JSON.parse(raw);
        return {
            lastSeenBundledForgeVersion: typeof parsed.lastSeenBundledForgeVersion === "string" ? parsed.lastSeenBundledForgeVersion : null,
            promptedForVersions: Array.isArray(parsed.promptedForVersions)
                ? parsed.promptedForVersions.filter((v) => typeof v === "string")
                : [],
        };
    }
    catch {
...
async function writeDriftCache(dir, cache) {
...
export async function checkBundledForgeDrift(opts) {
    ...
    if (last && last !== current && !cache.promptedForVersions.includes(current)) {
        opts.notify(`forge: bundled forge plugin changed (${last} → ${current}). ` +
            `Run /forge:update to apply project migrations from v${last} to v${current}.`, "info");
 ...
 * partially).
 */
export function composeUpdateKickoff(updateMd, bundleRoot, preflight) {
    ...
    if (patched.includes(canaryAnchor)) {
        patched = patched.replace(canaryAnchor, ` forge-cli: IS_CANARY is ALWAYS true — the bundled payload is never in the Claude Code plugin directory. ` +
    ...
    if (patched.includes(distAnchor)) {
        patched = patched.replace(distAnchor, distAnchor +
...
export function registerForgeUpdateCommand(pi, opts) {
    pi.registerCommand("forge:update", {
        ...
        async handler(_args, ctx) {
            ...
            const upgrade = opts.upgradeRunner ?? ((spec) => runUpgrade(spec));
            ...
            let cliNewVersion;
            if (method !== "global") {
                ctx.ui.notify(`forge:update — install method '${method}' is not eligible for guided npm upgrade. ` +
                    `To upgrade the CLI manually: npm i -g ${PKG_NAME}@latest. ` +
                    "Proceeding to project update…", "warning");
            }
            else {
                ...
                if (!release) {
                    ctx.ui.notify("forge:update — could not reach the npm registry to check for updates. " +
                        ...
                        if (!proceed) {
                            ...
                            if (!result.ok) {
                                ctx.ui.notify(`forge:update — npm i -g failed: ${truncate(result.stderr, 400)}. ` +
                                    ...
                                    "Proceeding to project update…", "error");
                            }
                            else {
                                upgraded = true;
            ...
            if (!bundledForgeVersion) {
                ctx.ui.notify("forge:update — could not determine bundled forge version. " +
                    "Run /forge:init first, or check the forge-payload installation.", "error");
                return;
            }
            ...
            try {
                ...
                ctx.ui.notify(`forge:update — bundled update.md not found at ${updateMdPath}. ` +
                    "The bundled payload may be incomplete. Run /forge:init to repair.", "error");
                return;
            }
            const preflight = {
                method,
...
//# sourceMappingURL=forge-update-command.js.map