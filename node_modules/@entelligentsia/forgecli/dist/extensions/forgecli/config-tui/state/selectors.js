...
import { PERSONA_META, TIER_PERSONAS, TIERS } from "../tier-meta.js";
export function getActiveView(state) {
    return state.view[state.view.length - 1];
}
export function listResolvedPersonas(state) {
    const out = new Map();
...
export function listPersonaPickerEntries(state) {
    const assignmentsByName = new Map(listResolvedPersonas(state).map((p) => [p.persona, p]));
    const catalogue = state.personaCatalogue.filter((p) => p !== "default").sort();
    const order = ["default", ...catalogue];
...
export function uniqueProviders(state) {
    const providers = new Set();
    for (const m of state.availableModels)
        providers.add(m.provider);
    for (const p of state.authenticatedProviders)
        providers.add(p);
    return [...providers].sort();
}
export function listPipelineOverrideSummaries(state) {
...
export function getPhaseOverride(state, pipeline, phaseRole) {
 ...
 */
export function sourceToLabel(rawSource, tier, isProject) {
    switch (rawSource) {
        case "L4-inline":
        case "L4-name":
        ...
        case "L2":
            if (tier)
                return `${TIER_LABELS[tier]} tier (project)`;
            return "Per-persona (project)";
        case "L1":
            if (tier)
                return `${TIER_LABELS[tier]} tier (global)`;
...
export function getPhaseTable(state) {
    ...
    return PHASE_PERSONA_TABLE.map((phase) => {
        ...
        const isProject = result.source === "L2" ||
...
export function getTierAssignment(state, tier) {
    const personas = TIER_PERSONAS[tier];
    const resolved = listResolvedPersonas(state);
    const resolvedMap = new Map(resolved.map((p) => [p.persona, p]));
    ...
    if (!allSame)
        return { status: "mixed" };
    const layer = first.source.endsWith("L2") ? "project" : "global";
...
export function getAllTierAssignments(state) {
    return TIERS.map((tier) => ({ tier, assignment: getTierAssignment(state, tier) }));
}
export function getScopedTierAssignment(state, tier, scope) {
    const personas = TIER_PERSONAS[tier];
    ...
    if (targetPresent.length === personas.length) {
        ...
        const allSame = targetEntries.every((e) => e !== undefined && e.provider === targetEntries[0].provider && e.model === targetEntries[0].model);
        if (allSame) {
            return {
                status: "set",
    ...
    if (targetPresent.length === 0) {
        ...
        if (otherPresent.length > 0) {
            ...
            const allSameOther = otherPresent.every((e) => e.provider === otherPresent[0].provider && e.model === otherPresent[0].model);
            if (allSameOther) {
                return {
...
export function getAllScopedTierAssignments(state, scope) {
...
export function getTierForPersona(persona) {
    return PERSONA_META[persona]?.tier;
}
/** Persona names in a tier. */
export function getPersonasInTier(tier) {
...
export function isConfigEmpty(state) {
...
export function personaSourceLabel(persona, source) {
