...
export function writePersonaEntry(buffer, layer, persona, entry) {
...
export function deletePersonaEntry(buffer, layer, persona) {
    const targetLayer = buffer[layer];
    const personaModels = { ...(targetLayer["persona-models"] ?? {}) };
    if (!(persona in personaModels))
        return buffer;
...
export function writeTierAssignment(buffer, layer, tier, entry) {
...
export function writePhaseOverride(buffer, pipelineName, phaseRole, override) {
...
export function clearPhaseOverride(buffer, pipelineName, phaseRole) {
    ...
    if (!existingPhase || existingPhase["model-override"] === undefined)
        return buffer;
    const project = { ...buffer.project };
    const pipelines = { ...(project.pipelines ?? {}) };
    const pipeline = { ...pipelines[pipelineName] };
    ...
    const hasPipelinePersonaModels = pipeline["persona-models"] !== undefined && Object.keys(pipeline["persona-models"]).length > 0;
