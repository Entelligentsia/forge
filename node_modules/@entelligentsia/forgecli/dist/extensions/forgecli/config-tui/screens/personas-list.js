...
export class PersonasListScreen {
    render(state, width, theme) {
        ...
        personas.forEach((p, i) => {
            ...
            lines.push(`  ${cur} ${emoji}${padRight(p.persona, personaCol)}  ${padRight(modelStr, modelCol)}  ${sourceCol} ${avail}`);
        });
        // Catalogue context
    ...
    handleInput(data, state) {
        ...
        if (matchesKey(data, "d")) {
            ...
            const target = personas[view.cursor];
            if (target) {
                const layer = target.source.endsWith("L2") ? "project" : "global";
...
//# sourceMappingURL=personas-list.js.map