 ...
 */
export function windowList(items, cursor, maxRows = 10) {
...
export function authBadgeFor(state, provider, theme) {
    return authBadge(state.authenticatedProviders.includes(provider), theme);
}
export function authStatusLine(state, theme) {
...
export function cascadeFooter(width, theme) {
    const m = (text) => theme.fg("muted", text);
    const a = (text) => theme.fg("accent", text);
    return [
        "",
...
export function tierBadge(tier, theme) {
    const labels = { heavy: "Heavy", standard: "Standard", light: "Light" };
    return theme.fg("accent", labels[tier] ?? tier);
}
/** Width-safe final guard: truncate every line to the given terminal width. */
export function safeLines(lines, width) {
 ...
 */
export function filterWithSearch(items, query, getText) {
    if (!query)
        return items;
 ...
 */
export function renderFilterLine(query, theme) {
 ...
 */
export function renderResultCount(shown, total, theme) {
    if (shown === total)
        return "";
 ...
 */
export function handleSearchKey(currentQuery, data) {
