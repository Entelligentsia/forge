...
import { authBadgeFor, filterWithSearch, handleSearchKey, renderFilterLine, renderResultCount, rule, safeLines, windowList, } from "./shared.js";
export class PersonaEditorScreen {
    render(state, width, theme) {
        ...
        if (view.step === "pick-provider") {
            ...
            if (filtered.length === 0) {
                ...
                if (win.aboveCount > 0)
                    lines.push(muted(`    ↑ ${win.aboveCount} more above`, theme));
                ...
                if (win.belowCount > 0)
                    lines.push(muted(`    ↓ ${win.belowCount} more below`, theme));
            }
    ...
    handlePickProviderInput(data, state, view) {
        const providers = uniqueProviders(state);
        const filtered = filterWithSearch(providers, view.searchQuery, (p) => p);
        ...
        if (matchesKey(data, Key.enter)) {
            ...
            if (provider)
                return { kind: "dispatch", action: { kind: "set-persona-provider", provider } };
            return { kind: "consumed" };
        }
        return { kind: "no-op" };
    }
    handlePickModelInput(data, state, view) {
        const models = state.availableModels.filter((m) => m.provider === view.provider);
        const filtered = filterWithSearch(models, view.searchQuery, (m) => `${m.id} ${m.provider}/${m.id}`);
        // Search input: backspace, ctrl+u/w, printable chars
        const searchResult = handleSearchKey(view.searchQuery, data);
        ...
        if (matchesKey(data, Key.enter)) {
            ...
            if (target)
                return { kind: "dispatch", action: { kind: "set-persona-model", model: target.id } };
            return { kind: "consumed" };
        }
        return { kind: "no-op" };
    }
    handlePickLayerInput(data, _state, view) {
        if (matchesKey(data, Key.up)) {
            return { kind: "dispatch", action: { kind: "cursor-move", delta: -1 } };
        }
        if (matchesKey(data, Key.down)) {
        ...
        if (matchesKey(data, "g")) {
            return { kind: "dispatch-seq", actions: [{ kind: "commit-persona-edit", layer: "global" }] };
        }
        if (matchesKey(data, "p")) {
            return { kind: "dispatch-seq", actions: [{ kind: "commit-persona-edit", layer: "project" }] };
        }
        if (matchesKey(data, Key.enter)) {
            const layer = view.cursor === 0 ? "project" : "global";
            return { kind: "dispatch-seq", actions: [{ kind: "commit-persona-edit", layer }] };
        }
        return { kind: "no-op" };
    }
}
//# sourceMappingURL=persona-editor.js.map