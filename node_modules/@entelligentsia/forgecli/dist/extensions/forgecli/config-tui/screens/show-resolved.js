...
import { getActiveView, getPhaseTable } from "../state/selectors.js";
 ...
 */
export function computeResolvedRows(state) {
    const pipelineNames = state.pipelineCatalogue ? state.pipelineCatalogue : ["default"];
...
export class ShowResolvedScreen {
    render(state, width, theme) {
        ...
        // ── Column headers ─────────────────────────────────────────────────
        lines.push(`  ${muted(padRight("Step", stepCol), theme)} ${muted(padRight("Persona", personaCol), theme)} ${muted(padRight("Model", modelCol), theme)} ${muted("Source", theme)}`);
        // ── Phase rows ─────────────────────────────────────────────────────
        const rows = getPhaseTable(state);
        const win = windowList(rows, view.cursor, 10);
        if (win.aboveCount > 0)
            lines.push(muted(`    ↑ ${win.aboveCount} row(s) above`, theme));
        ...
        if (win.belowCount > 0)
            lines.push(muted(`    ↓ ${win.belowCount} row(s) below`, theme));
        // ── Cascade footer ─────────────────────────────────────────────────
...
//# sourceMappingURL=show-resolved.js.map