...
export class OverrideEditorScreen {
    ...
    handlePickTypeInput(data, _state, view) {
    ...
    handlePickNameInput(data, state, view) {
        const personas = listResolvedPersonas(state);
        const filtered = filterWithSearch(personas, view.searchQuery, (p) => `${p.persona} ${p.provider}:${p.model} ${p.provider}/${p.model}`);
    ...
    handlePickProviderInput(data, state, view) {
        ...
        if (matchesKey(data, Key.enter)) {
            ...
            if (provider)
                return { kind: "dispatch", action: { kind: "set-override-provider", provider } };
            return { kind: "consumed" };
        }
        return { kind: "no-op" };
    }
...
//# sourceMappingURL=override-editor.js.map