...
export class DashboardComponent {
    ...
    spinnerTimer;
    _rerender;
    constructor(tree, tui, theme, done) {
        this.tree = tree;
        this.tui = tui;
        this.theme = theme;
        this.done = done;
    ...
    ensureSpinnerTimer() {
        ...
        this.spinnerTimer = setInterval(() => {
            const anyRunning = this.tree.getRoots().some((r) => r.status === "running" || r.status === "cancelling");
            if (!anyRunning) {
    ...
    renderDetailPanel(node, width) {
        ...
        if (node.kind === "orchestrator" && node.children.length > 0) {
            ...
            for (const child of children) {
                ...
                lines.push(` ${cglyph} ${child.label}${dim(cmodel)}${dim(cmetricsPart)}`);
            }
            lines.push("");
        }
    ...
    renderCancelConfirm(width, targetId) {
