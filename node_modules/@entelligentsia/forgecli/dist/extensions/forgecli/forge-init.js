...
export function registerForgeInit(pi) {
    ...
    pi.registerCommand("forge:init", {
        ...
        async handler(args, ctx) {
            ...
            if (flags.migrate) {
                ...
                const syntheticPi = {
                    ...
                    registerCommand: (name, def) => {
                        if (name === "forge:migrate")
                            migrateHandler = def.handler;
                    },
                };
            ...
            // Phase 1 — Collect
            if (startPhase <= 1) {
                const phase1Result = await runPhase1(cwd, bundleRoot, toolsRoot, configCache, ctx, sendToAgent, () => ctx.waitForIdle(), isNonInteractive);
                if (phase1Result === "abort")
                    return;
                // Inter-phase gate: let user review before Phase 2
                if (!isNonInteractive()) {
                    const proceed = await ctx.ui.confirm("Phase 1 complete — proceed?", "Phase 1 deliverables verified. Continue to Phase 2: Discover (KB generation)?");
                    if (!proceed) {
            ...
            // Phase 2 — Discover
            if (startPhase <= 2) {
                const phase2Result = await runPhase2(cwd, bundleRoot, toolsRoot, projectName, configCache, ctx, sendToAgent, () => ctx.waitForIdle(), isNonInteractive);
                if (phase2Result === "abort")
                    return;
                // Inter-phase gate: let user review before Phase 3
                if (!isNonInteractive()) {
                    const proceed = await ctx.ui.confirm("Phase 2 complete — proceed?", "Phase 2 deliverables verified. Continue to Phase 3: Materialize (workflow generation)?");
                    if (!proceed) {
            ...
            if (!finalP3.ok) {
                ...
                incompleteDetail.push(`  → Recover: \`/forge:rebuild\` (re-runs substitute-placeholders against current config), or delete .forge/init-progress.json and re-run /forge:init.`);
            }
            const report = [
                ``,
                `╔══════════════════════════════════════════════════════════════╗`,
