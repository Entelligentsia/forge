...
import { loadSkillsFromDir, VERSION as PI_VERSION } from "@earendil-works/pi-coding-agent";
...
import { buildForgeAwarenessMsg, buildMultiPluginMsg, buildPendingMigrationMsg, buildVersionDriftMsg, syncForgeRootAndRef, } from "./hooks/check-update.js";
...
export default async function forgecli(pi) {
    ...
    if (process.env.FORGE_SPIKE_R2 === "1") {
        ...
        const mod = (await import(subagentPath));
        mod.default(pi);
        if (process.env.FORGE_SPIKE_R2_DEBUG === "1") {
    ...
    pi.on("session_start", async (_event, ctx) => {
        if (!ctx.hasUI)
            return; // headless mode — no banners
        ...
        if (collision.collides && collision.colliderPath !== null && !wasCollisionSeen(collision.colliderPath)) {
            ...
            ctx.ui.notify(`forge: collision detected — another 'forge' binary found at ${collision.colliderPath}. ` +
                "Use 'forgecli' or '4ge' to disambiguate.", "warning");
        }
        if (!forgeRoot) {
        ...
        if (forgeConfig) {
            ...
            try {
                ...
                if (awarenessMsg)
                    ctx.ui.notify(awarenessMsg, "info");
            }
            catch {
                /* non-fatal */
            }
            ...
            try {
                ...
                if (multiPluginMsg)
                    ctx.ui.notify(multiPluginMsg, "info");
            }
            catch {
                /* non-fatal */
            }
            ...
            try {
                const driftMsg = buildVersionDriftMsg(forgeConfig.configPath, PKG_VERSIONS.bundledForgeVersion ?? "");
                if (driftMsg)
                    ctx.ui.notify(driftMsg, "warning");
            }
            catch {
                /* non-fatal */
            }
            ...
            try {
                ...
                if (switchMsg)
                    ctx.ui.notify(switchMsg, "warning");
            }
            catch {
                /* non-fatal */
            }
            ...
            try {
                ...
                if (pendingMsg)
                    ctx.ui.notify(pendingMsg, "warning");
            }
            catch {
                /* non-fatal */
            }
        ...
        if (forgeRoot && forgeConfig) {
            try {
                ...
                if (credBanner)
                    ctx.ui.notify(credBanner, "warning");
            }
            catch (err) {
            ...
            try {
                ...
                if (result.diagnostics.length > 0) {
                    for (const diag of result.diagnostics) {
                        console.warn(`[forge-cli] skill diagnostic: ${diag.type} ${diag.path ?? "(unknown)"}: ${diag.message}`);
                    }
                }
                if (result.skills.length !== EXPECTED_SKILL_COUNT) {
                    console.warn(`[forge-cli] expected ${EXPECTED_SKILL_COUNT} bundled skills, loaded ${result.skills.length}`);
                }
            }
            catch (err) {
