...
import { compressStoreQuery, compressEntity, compressEntityList, compressValidateStore, countTokens, } from "@entelligentsia/forge-compress";
import { buildForgeArtifact } from "./forge-artifact-tool.js";
import { execFileAsync } from "./lib/exec-helpers.js";
import { assertBugStatusOwnership, assertOrchestratorOnlyEmit, assertPhaseOwnership, PhaseOwnershipError, } from "./subagent/phase-guard.js";
 ...
 */
export async function runCjs(toolPath, argv, signal, timeoutMs, projectRoot) {
    const opts = {
        cwd: projectRoot,
        encoding: "utf8",
        timeout: timeoutMs,
...
function okResult(text, compression) {
    return {
 ...
 */
export function getSubagentTools(defs, _personaName) {
 ...
 */
function registerForgeToolDiscipline(pi, toolDir) {
...
function buildForgeCollate(toolDir, projectRoot) {
    return {
        ...
        parameters: Type.Object({
            ...
            purgeEvents: Type.Optional(Type.Boolean({
        ...
        async execute(_toolCallId, _params, signal) {
            ...
            const argv = [];
            if (params.sprintId)
                argv.push(params.sprintId);
            if (params.purgeEvents)
                argv.push("--purge-events");
            if (params.dryRun)
                argv.push("--dry-run");
            try {
...
function buildForgeStore(toolDir, projectRoot) {
    return {
        ...
        promptSnippet: "Use forge_store for store CRUD. Direct cjs exec — no shell, no bash. " +
            "Canonical write form is 2-positional: {command:'write', args:['<entity>','<json>']}. " +
            "Call forge_store_template first to get the json shape.",
        parameters: Type.Object({
            command: Type.String({
                description: "store-cli subcommand: write|read|list|delete|update-status|emit|merge-sidecar|record-usage|purge-events|write-collation-state|validate|set-summary|set-bug-summary|progress|progress-clear|describe|template",
            }),
            args: Type.Array(Type.String(), {
                description: "Positional + flag args after the subcommand. Examples: " +
        ...
        async execute(_toolCallId, _params, signal) {
            ...
            try {
                if (params.command === "set-bug-summary" || params.command === "set-summary") {
                    ...
                    if (namedPhase)
                        assertPhaseOwnership(`forge_store ${params.command}`, namedPhase);
                }
                ...
                if (err instanceof PhaseOwnershipError)
...
function buildForgeStoreDescribe(toolDir, projectRoot) {
    return {
        ...
        promptSnippet: "Use forge_store_describe to fetch the JSON Schema for sprint/task/bug/event/feature before constructing a record.",
        parameters: Type.Object({
...
function buildForgeStoreTemplate(toolDir, projectRoot) {
    return {
        ...
        promptSnippet: "Use forge_store_template to get a canonical sample for sprint/task/bug/event/feature — copy the shape, replace placeholders.",
        parameters: Type.Object({
            entity: STORE_ENTITIES,
        }),
        async execute(_toolCallId, _params, signal) {
...
function buildForgeStoreQuery(toolDir, projectRoot) {
    return {
        ...
        description: "Search / find / query the Forge store. Wraps `store-cli.cjs` query/nlp/schema dispatch (which delegates to store-query.cjs). " +
        ...
        promptSnippet: 'Use forge_store_query to search the store. nlp "<intent>" finds tasks/bugs/sprints/features by natural language; ' +
        ...
        parameters: Type.Object({
            command: Type.Union([Type.Literal("query"), Type.Literal("nlp"), Type.Literal("schema")], {
                description: "store-query subcommand: query (flag-based filters), nlp (natural-language intent), schema (project meta reference).",
            }),
            args: Type.Array(Type.String(), {
                description: "Positional and flag arguments after the subcommand. " +
...
// ── forge_preflight ─────────────────────────────────────────────────────────
function buildForgePreflight(toolDir, projectRoot) {
    return {
        name: "forge_preflight",
        label: "Forge Preflight Gate",
        description: "Run the pre-flight gate check for a phase. Wraps forge/tools/preflight-gate.cjs. " +
            "Returns exit 0 if the phase is safe to proceed, exit 1 with a reason if blocked.",
        promptSnippet: "Use forge_preflight instead of shelling out to preflight-gate.cjs. " +
        ...
        async execute(_toolCallId, _params, signal) {
            ...
            const argv = ["--phase", params.phase];
            if (params.task)
                argv.push("--task", params.task);
            if (params.bug)
                argv.push("--bug", params.bug);
            try {
                const { stdout } = await runCjs(toolPath, argv, signal, 10_000, projectRoot);
...
// ── forge_banner ────────────────────────────────────────────────────────────
function buildForgeBanner(toolDir, projectRoot) {
    return {
        name: "forge_banner",
        label: "Forge Banner",
        description: "Display a decorative phase banner. Wraps forge/tools/banners.cjs. " +
