...
function writeSentinel(sentinel, projectPrefix) {
    try {
        const dir = path.dirname(sentinel);
        fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(sentinel, JSON.stringify({ firedAt: new Date().toISOString(), projectPrefix }, null, 2) + "\n", "utf8");
    }
    catch {
 ...
 */
export function createPostInitHookHandler(pi) {
    return async function postInitHookHandler(event, ctx) {
        ...
        try {
            ...
            if (err instanceof WorkflowLoaderError && err.code === "missing_file") {
                ctx.ui.notify(`× post-init hook: enhance workflow not found at ${WORKFLOW_REL_PATH}; ` +
                    "run /forge:init or /forge:rebuild first. Skipping phase-1 enhance.", "info");
            }
            else {
                const e = err;
        ...
        try {
            ...
            const e = err;
            ctx.ui.notify(`× post-init hook: failed to trigger /forge:rebuild --enrich: ${e.message ?? "unknown"}`, "error");
 ...
 */
export function registerPostInitHook(pi) {
    onSyntheticEvent("init-complete", createPostInitHookHandler(pi));
}
//# sourceMappingURL=post-init-hook.js.map