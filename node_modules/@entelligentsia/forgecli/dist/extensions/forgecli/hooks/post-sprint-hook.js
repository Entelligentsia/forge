...
export function isSprintId(id) {
    return SPRINT_ID_REGEX.test(id);
}
// ── Sentinel helpers ──────────────────────────────────────────────────────────
function sentinelPath(cwd, sprintId) {
...
function writeSentinel(sentinel, sprintId) {
    try {
        const dir = path.dirname(sentinel);
        fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(sentinel, JSON.stringify({ firedAt: new Date().toISOString(), sprintId }, null, 2) + "\n", "utf8");
    }
    catch {
 ...
 */
export function createPostSprintHookHandler(pi) {
    return async function postSprintHookHandler(event, ctx) {
        const { sprintId, cwd } = event;
        // 1. Sprint-ID regex gate — filter out bug IDs
        if (!isSprintId(sprintId)) {
            ctx.ui.notify(`post-sprint hook: sprintId "${sprintId}" does not match sprint-ID shape ` +
                `(^[A-Z]+-S\\d+$); skipping enhance trigger (bug-fix collate, not sprint collate).`, "info");
            return;
        }
        ...
        try {
            ...
            if (err instanceof WorkflowLoaderError && err.code === "missing_file") {
                ctx.ui.notify(`× post-sprint hook: enhance workflow not found at ${WORKFLOW_REL_PATH}; ` +
                    "run /forge:init or /forge:rebuild first. Skipping phase-2 enhance.", "info");
            }
            else {
                const e = err;
        ...
        try {
            ...
            const e = err;
            ctx.ui.notify(`× post-sprint hook: failed to trigger /forge:rebuild --enrich: ${e.message ?? "unknown"}`, "error");
 ...
 */
export function registerPostSprintHook(pi) {
    onSyntheticEvent("sprint-collate-complete", createPostSprintHookHandler(pi));
}
//# sourceMappingURL=post-sprint-hook.js.map