...
import { composeChangelogSummary, detectInstallMethod, fetchChangelog, getNpmGlobalRoot, isUpgrade, runUpgrade, } from "../extensions/forgecli/forge-update-command.js";
...
function printInstallMethodError(method) {
    process.stderr.write(`forge update: install method '${method}' is not eligible for guided upgrade.\n` +
...
export async function runUpdate(args, opts) {
    ...
    if (!release) {
        process.stderr.write("forge update: could not reach the npm registry to check for updates.\n" +
    ...
    if (!versionSpec && !isUpgrade(current, release.version)) {
        process.stdout.write(`forge update: already at the latest version (${current}; latest published: ${release.version}).\n`);
        return 0;
    }
    ...
    const doUpgrade = yes || process.env.FORGE_NON_INTERACTIVE === "1"
        ? true
    ...
    if (!result.ok) {
        process.stderr.write(`forge update: npm i -g failed:\n${result.stderr}\n` +
            "Check the error above; you may need elevated permissions to install globally.\n");
        return 1;
    }
    process.stdout.write(`forge update: installed ${PKG_NAME}@${release.version}.\n` +
