...
// auth migration.
import { AuthStorage, formatNoModelsAvailableMessage, getAgentDir, getProviderLoginHelp, ModelRegistry, runMigrations, SettingsManager, } from "@earendil-works/pi-coding-agent";
/**
 ...
 * launch.
 */
export async function runDoctorProbe(versions, cwd = process.cwd()) {
...
export function formatDoctorReport(r) {
    ...
    if (r.migrations.deprecationWarnings.length > 0) {
        ...
        for (const w of r.migrations.deprecationWarnings)
            lines.push(`  ⚠ ${w}`);
    }
 ...
 */
export async function runDoctor(args, versions) {
