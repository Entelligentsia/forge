...
1. Check whether the project root contains a `.gitignore` file:
   ...
   ```
   If absent, skip this step entirely (no gitignore convention).

2. If `.gitignore` exists, check whether `.forge/store/events/` is already
   ignored. Treat any of the following lines as a match (each matched as a
   literal substring on any non-comment, non-blank line):
   - `.forge/store/events/`
   - `.forge/store/events`
   - `.forge/store/`
   - `.forge/`
...
3. If no match is found, **unconditionally** append:
   ```
   ...
   No prompt. This is unconditional — the transient event logs should never be
...
1. Prompt:
   ```
   No agent instruction file found at project root. Create a minimal CLAUDE.md
   with links to the Forge knowledge base? [Y/n]
   ```

2. Default is "Y" (most users want the link). If Y:
   ...
   Emit `〇 Created CLAUDE.md with KB links.`

3. If user declines: emit `〇 KB not linked — run /forge:refresh-kb-links after creating CLAUDE.md.`

If one already exists: skip this step. Do NOT modify existing files — this
avoids regressing KB-link bloat (see GH #73).
