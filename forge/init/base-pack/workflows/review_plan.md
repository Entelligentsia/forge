...
1. Read Review Loop Context:
   ...
   - If this is the final iteration (`N == M`) and the verdict is `Revision Required`,
     ...
     - Restart from review: `/forge:review-plan {record_id}`
     ```
...
5. Finalize:
   - Transitions:
     - **Task mode** — predecessor must be `planned`.
       ...
       Update status: `node .forge/tools/store-cli.cjs update-status task {record_id} status plan-approved` (if Approved) or `... status plan-revision-required` (if Revision Required)
