...
// 5. Commands — structure-manifest expectation set for check-structure / health.
//
// Unified command model (FORGE-S32-T06): the former two-track split (a
// per-project generated base-pack/commands/ tree PLUS a plugin forge/commands/
// tree) was collapsed into a SINGLE unified tree at forge/forge/commands/,
// installed verbatim to .claude/commands/{PREFIX}/ by the bootstrap manifest's
// lone `commands` entry. There is no longer a per-project command-generation
// step.
//
// COMMAND_NAMES remains the curated set of sprint-workflow command files that
// /forge:health (check-structure.cjs) treats as REQUIRED in an installed
// project's .claude/commands/{PREFIX}/. It is the structure-check expectation
// set, NOT a generation manifest — the unified tree ships additional
// plugin-utility commands (health, config, status, ...) that check-structure
// does not require. It stays at 14 entries (v1.0: collate and enhance removed
// from the required user-facing surface — T03).
