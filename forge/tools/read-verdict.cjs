 ...
 */
function readVerdict({ record, phase }) {
  if (!record || typeof record !== 'object') {
    return { verdict: null, source: 'no-record', key: null };
  }
  const spec = PHASE_VERDICT_SOURCE[phase];
  if (spec === undefined) {
    // Unknown phase — defensive defaults match the store-key naming convention
    // (hyphen → underscore). Caller will likely treat null as missing.
  ...
  if (spec === STATUS_SOURCE) {
    // Approve-style phase: verdict source is task.status.
    return {
...
module.exports = { readVerdict, PHASE_VERDICT_SOURCE, STATUS_SOURCE, ALLOWED_VERDICTS };
...
if (require.main === module) {
  ...
  const out = readVerdict({ record, phase: args.phase });
