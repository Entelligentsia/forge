# /forge:rebuild
**Category:** Forge plugin cmd
**Run from:** Any Forge-initialised project dir
## Purpose
...
Use `--enrich` to run the enhancement agent (KB enrichment + drift detection), which was previously handled by the removed `/forge:enhance` command.

## Invocation
```
/forge:rebuild                              # default: workflows + commands + templates + personas + skills
/forge:rebuild --enrich                     # run enhancement agent (KB enrichment + drift detection)
/forge:rebuild workflows                    # atomic workflows + orchestration
/forge:rebuild workflows plan_task          # single workflow file
/forge:rebuild workflows:plan_task          # same — colon form (from migration entries)
/forge:rebuild personas                     # all persona files
/forge:rebuild personas engineer            # single persona file
/forge:rebuild skills                       # all skill files
/forge:rebuild skills engineer              # single skill file
/forge:rebuild commands                     # slash command wrappers
/forge:rebuild templates                    # doc templates
/forge:rebuild templates PLAN_TEMPLATE      # single template file
/forge:rebuild knowledge-base               # all three KB sub-targets (merge mode)
...
/forge:rebuild tools                        # engineering/tools/
```
...
**When to run:** After significant KB changes that should be reflected in plan/review doc formats.
...
- Additive only — existing sections are never deleted or overwritten
...
| `business-domain` | db | `entity-model.md` | New ORM models or schema tables added |
| `stack-checklist` | stack + testing | `stack-checklist.md` | New libraries adopted mid-project |

## Common behaviour (all categories)

- Shows a unified diff between current and rebuilt content before writing
...
After rebuild, Forge reconstructs:
- **Persona pack** (`.forge/cache/persona-pack.json`) — compact persona/skill references for subagent prompts
- **ctx pack** (`.forge/cache/context-pack.md`, `.forge/cache/context-pack.json`) — architecture summary for subagent prompts
...
The KB updates automatically via retrospective writeback. The generated workflows do not — they are a snapshot that only changes on explicit rebuild.

```mermaid
flowchart LR
    RET["/forge:retro"] -->|auto| KB["Knowledge Base"]
    KB -.->|manual trigger| RW["/forge:rebuild\nworkflows"]
    RW --> WF["Updated workflows"]
```

Run `rebuild workflows` every few sprints, or after a retrospective that revealed significant new patterns.

| Situation | Command |
|---|---|
| After adding a custom pipeline | `rebuild workflows` |
| After a significant retrospective | `rebuild workflows` |
| Health detects orphaned entities | `rebuild knowledge-base business-domain` |
| Health detects new subsystems | `rebuild knowledge-base architecture` |
| Health detects new libraries | `rebuild knowledge-base stack-checklist` |
| Health detects stale personas | `rebuild personas` |
| Health detects stale skills | `rebuild skills` |
| New tool spec added | `rebuild tools` |
| After Forge plugin update | Use `/forge:update` — it handles rebuild automatically |
| KB enrichment + drift detection | `rebuild --enrich` |

## Related commands
| Command | Purpose |
|---|---|
| [`/forge:health`](health.md) | Detect what needs rebuilding |
| [`/forge:update`](update.md) | Post-plugin-update rebuild (version-aware) |
| [`/forge:add-pipeline`](add-pipeline.md) | Add a pipeline (then run `rebuild workflows`) |
