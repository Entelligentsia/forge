...
Interviews the user to capture structured sprint requirements. Produces `SPRINT_REQUIREMENTS.md` — the document that `/forge:plan-sprint` reads as its primary input.
...
The Architect tells the user explicitly: *"Run `/forge:plan-sprint` when you're ready to break these into tasks."*
...
```mermaid
flowchart LR
    A([new-sprint\n← you are here]) -->|SPRINT_REQUIREMENTS.md| B([plan-sprint])
    B --> C([run-sprint])
    C --> D([retro])
    D -.->|KB updates| A
