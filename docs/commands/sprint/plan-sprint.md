...
**Role:** Architect  
**Lifecycle position:** After `/forge:new-sprint`; before `/forge:run-sprint`.
...
| Source | Purpose |
|---|---|
| `engineering/sprints/{SPRINT_ID}/SPRINT_REQUIREMENTS.md` | Primary input — **must exist**. Stops and directs to `/forge:new-sprint` if absent. |
...
| Situation | Behaviour |
|---|---|
| Requirements document missing | Stop; instruct user to run `/forge:new-sprint` |
