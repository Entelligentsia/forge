...
If a workflow step consistently caused friction or a template section was consistently skipped, the retrospective proposes a specific edit. These are proposals — the user decides whether to apply them via `/forge:rebuild workflows`.
...
```bash
/forge:rebuild workflows   # if retrospective revealed significant new patterns
```
